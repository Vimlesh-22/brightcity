...
const Header = () => {
  ...
  return (
    <header className="bg-background shadow-md border-b border-border">
      ...
      <div className="bg-primary text-primary-foreground text-sm py-2 px-4 md:px-8">
        <div className="container mx-auto flex justify-between items-center">
          <div className="hidden md:flex items-center gap-4">
            <span className="cursor-pointer hover:text-primary-foreground/80 transition-colors duration-300 hover:scale-105">Track Order</span>
            <span className="cursor-pointer hover:text-primary-foreground/80 transition-colors duration-300 hover:scale-105">Customer Support</span>
            <span className="cursor-pointer hover:text-primary-foreground/80 transition-colors duration-300 hover:scale-105">Professional Login</span>
          </div>
      ...
      <div className="py-4 px-4 md:px-8">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center gap-8">
            <Link href="/" className="text-3xl font-bold text-primary brand-text">
              BrightCity
            </Link>
          </div>

          <div className="hidden md:flex flex-grow max-w-xl items-center border border-border rounded-full overflow-hidden">
            <input
              type="text"
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="md:hidden bg-background pb-4 px-4 border-b border-border">
      ...
      {/* Category navigation */}
      <nav className="hidden md:block bg-secondary py-2 px-4 md:px-8 border-t border-b border-border">
        <div className="container mx-auto flex justify-center items-center">
          <div className="flex items-center gap-6 overflow-x-auto">
            {categories.map((category) => (
              <Link key={category.name} href={category.href} className="text-secondary-foreground hover:text-primary whitespace-nowrap transition-colors duration-300 pb-1 border-b-2 border-transparent hover:border-primary">
                  {category.name}
              </Link>
            ))}
          </div>
